...
export async function runGovernedDemo() {
  ...
  const task = await prisma.task.create({ data: { title: `Governed vertical slice ${new Date().toISOString()}`, goal: 'Prove implementation and independent review GitHub principals end to end.', status: 'RUNNING', repositoryId: repository.id, agentId: implementationAgent.id, initiatorId: initiator.id, requiresHumanApproval: false } });
  ...
  try {
    ...
    await prisma.auditEvent.create({ data: { taskId: task.id, executionId: execution.id, eventType: 'execution.completed', actorType: 'system', actorId: 'gitagent', payload: { policyVersion: '2026-09-16.1', result: 'success', pullRequestNumber: change.pullRequestNumber, selfApprovalDenied: selfApproval.allowed === false, independentReviewPrincipal: independentReview.reviewGitHubApp ?? null, independentApprovalSubmitted: independentApproval.approvalSubmitted === true } } });

    return { taskId: task.id, executionId: execution.id, repository: branch.repository, branch: branch.branch, pullRequestNumber: change.pullRequestNumber, pullRequestUrl: change.pullRequestUrl, selfApproval, independentReview, independentApproval };
  } catch (error) {
