...
export async function runGovernedDemo() {
  ...
  const task = await prisma.task.create({ data: { title: `Governed vertical slice ${new Date().toISOString()}`, goal: 'Prove implementation, independent review, adversarial controls, scoped capabilities, and trust gating end to end.', status: 'RUNNING', repositoryId: repository.id, agentId: implementationAgent.id, initiatorId: initiator.id, requiresHumanApproval: trustState === 'RESTRICTED' } });
