...
type AgentPlan = { summary: string; files: Array<{ path: string; reason: string }>; proposedChanges: Array<{ path: string; content: string; message: string }> };
...
async function requireGrant(agentId: string, capability: string, repositoryId: string, fullName: string, taskId: string, executionId: string) {
  const grants = await prisma.capabilityGrant.findMany({ where: { agentId, capability, effect: 'ALLOW', OR: [{ resource: '*' }, { resource: repositoryId }, { resource: fullName }, { resource: `task:${taskId}` }], AND: [{ OR: [{ expiresAt: null }, { expiresAt: { gt: new Date() } }] }] } });
...
function pickCandidateFiles(goal: string, files: RepoFile[]) {
  const words = goal.toLowerCase().split(/[^a-z0-9_.-]+/).filter((word) => word.length > 2);
  const ignored = new Set(['node_modules', '.next', 'dist', 'build', '.git', 'coverage']);
  return files.filter((file) => file.type === 'blob' && file.size < 150_000).filter((file) => !file.path.split('/').some((part) => ignored.has(part))).map((file) => ({ ...file, score: words.reduce((score, word) => score + (file.path.toLowerCase().includes(word) ? 3 : 0), 0) + (/(src|app|lib|components|pages|api)\//.test(file.path) ? 1 : 0) })).sort((a, b) => b.score - a.score || a.path.localeCompare(b.path)).slice(0, 12);
}
...
function validatePlan(plan: AgentPlan, allowedPaths: Set<string>) {
  ...
  if (invalid.length > 0) throw new Error(`Generated plan attempted to modify files outside inspected repository context: ${invalid.map((item) => item.path).join(', ')}`);
}

function mergeChanges(current: Array<{ path: string; content: string; message: string }>, repair: Array<{ path: string; content: string; message: string }>) {
...
async function markReadyForHumanApproval(input: { taskId: string; executionId: string; repository: string; pullRequestNumber: number; reviewerAgentId?: string | null; reviewSubmitted?: boolean }) {
  await prisma.$transaction([
    prisma.execution.update({ where: { id: input.executionId }, data: { status: 'SUCCEEDED', finishedAt: new Date() } }),
    prisma.task.update({ where: { id: input.taskId }, data: { status: 'WAITING_APPROVAL' } }),
    prisma.auditEvent.create({ data: { taskId: input.taskId, executionId: input.executionId, eventType: 'agent.execution.completed', actorType: 'system', actorId: 'gitagent', payload: { repository: input.repository, pullRequestNumber: input.pullRequestNumber, reviewerAgentId: input.reviewerAgentId ?? null, reviewSubmitted: input.reviewSubmitted ?? false, outcome: 'WAITING_APPROVAL', reasonCode: 'execution.ready_for_human_merge_decision', policyVersion: POLICY_VERSION } } }),
  ]);
}

export async function runRepositoryAgent(executionId: string) {
  const execution = await prisma.execution.findUnique({ where: { id: executionId }, include: { task: { include: { repository: { include: { user: true } }, approvals: true } }, agent: true, workspace: true } });
  ...
  for (const file of candidates.slice(0, 8)) {
    try {
      const result = await github<{ content?: string; encoding?: string }>(`${API}/repos/${repo.owner}/${repo.name}/contents/${encodeURIComponent(file.path)}?ref=${encodeURIComponent(repo.defaultBranch)}`, installation.token);
      if (result.encoding === 'base64' && result.content) inspected.push({ path: file.path, content: Buffer.from(result.content.replace(/\n/g, ''), 'base64').toString('utf8') });
    } catch {}
  }
  ...
  const plan = await generateCodingPlan({ goal: task.goal, repository: fullName, defaultBranch: repo.defaultBranch, files: inspected });
  const allowedPaths = new Set(inspected.map((file) => file.path));
  validatePlan(plan, allowedPaths);
  await prisma.auditEvent.create({ data: { taskId: task.id, executionId, eventType: 'agent.plan.created', actorType: 'agent', actorId: agent.id, payload: { summary: plan.summary, files: plan.files, proposedChangePaths: plan.proposedChanges.map((change) => change.path), policyVersion: POLICY_VERSION } } });
  ...
  let validation: ValidationResult = await validateRepositorySnapshot({ owner: repo.owner, name: repo.name, branch: repo.defaultBranch, files: proposedChanges.map((change) => ({ path: change.path, content: change.content })) });
  await prisma.auditEvent.create({ data: { taskId: task.id, executionId, eventType: 'agent.validation.completed', actorType: 'agent', actorId: agent.id, payload: { repository: fullName, attempt: 0, projectType: validation.projectType, passed: validation.passed, commands: validation.commands, policyVersion: POLICY_VERSION } } });

  let repairAttempts = 0;
  while (!validation.passed && repairAttempts < MAX_REPAIR_ATTEMPTS) {
    repairAttempts += 1;
    await prisma.auditEvent.create({ data: { taskId: task.id, executionId, eventType: 'agent.repair.started', actorType: 'agent', actorId: agent.id, payload: { repository: fullName, attempt: repairAttempts, failedCommands: validation.commands.filter((item) => item.exitCode !== 0).map((item) => item.command), policyVersion: POLICY_VERSION } } });
    const currentFiles = inspected.map((file) => { const changed = proposedChanges.find((item) => item.path === file.path); return { path: file.path, content: changed?.content ?? file.content }; });
    const repair = await generateValidationRepair({ goal: task.goal, repository: fullName, files: currentFiles, validation: validation.commands, attempt: repairAttempts });
    validatePlan(repair, allowedPaths);
    if (repair.proposedChanges.length === 0) break;
    proposedChanges = mergeChanges(proposedChanges, repair.proposedChanges);
    validation = await validateRepositorySnapshot({ owner: repo.owner, name: repo.name, branch: repo.defaultBranch, files: proposedChanges.map((change) => ({ path: change.path, content: change.content })) });
    await prisma.auditEvent.create({ data: { taskId: task.id, executionId, eventType: 'agent.repair.completed', actorType: 'agent', actorId: agent.id, payload: { repository: fullName, attempt: repairAttempts, summary: repair.summary, changedFiles: repair.proposedChanges.map((change) => change.path), validationPassed: validation.passed, projectType: validation.projectType, commands: validation.commands, policyVersion: POLICY_VERSION } } });
  }
